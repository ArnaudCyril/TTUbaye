...
if(isset($_POST["action"])&& $_POST["action"]=="Modifier")
		...
		$req10="update prixc set libelle='$cate' , prix='$prix' where libelle='$old';";
...
<br><h3 >Liste des prix d'une licence Compétition</h3></center></br></br>
<div id="infoPrix">
...
{ 
$req="select * from prixc order by rang;";
$res=mysql_query($req);
...
?>
<tr>
		<form name="Modif" action="tarifC.php" method="POST">
		<input type="hidden" name="inputOldcate" value="<?php echo $categorie?>"/>
		<th height=45 <?php if($nb%2==0):?>bgcolor='#b9c9fe'<?php else:?>bgcolor='#b9c9fe'<?php endif?>><?php echo $categorie?></th>
		<th height=45 <?php if($nb%2==0):?>bgcolor='#b9c9fe'<?php else:?>bgcolor='#b9c9fe'<?php endif?>><input type="text" name="inputPrix" value="<?php echo $prix?>"/>€</th>
		<td height=45 <?php if($nb%2==0):?>bgcolor='#b9c9fe'<?php else:?>bgcolor='#d3ddff'<?php endif?>><input type="submit" name="action" value="Modifier"/></td>
		<?php $nb++; ?>
		</form>
</tr>
<?php }
