...
function createDiapo($tabPhoto,$tabTitle,$idPost)
{
	...
	$("#buttonContainer"+id).children().css("color","#700"); var noSlide=opts.currSlide+1;$("."+id+"buttonGoToDiapo"+noSlide).css("color","#FF0000"); }</script>';
	  ...
	  $html.='<script>function gereDiapo(no,etat){if(etat=="pausediapo"){$("#diapo"+no).cycle("pause");$(".gereButton"+no).attr("id","playdiapo");$(".gereButton"+no).text("");$(".gereButton"+no).append(\'<img src="./images/icones/play.png"  id="buttonpause">\')}else{$("#diapo"+no).cycle("resume");$(".gereButton"+no).attr("id","pausediapo");$(".gereButton"+no).text("");$(".gereButton"+no).append(\'<img src="./images/icones/pause.png"  id="buttonpause">\')}}</script>';
	 ...
	 $html.='</div></br><center><span id="buttonContainer'.$idPost.'"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<label onclick="gereDiapo('.$idPost.',this.id)" class="gereButton'.$idPost.'" id="pausediapo" ><img src="./images/icones/pause.png"  id="buttonpause"></label></center>';
	 echo $html;
...
if(isset($_POST["action"]))
{
		...
		{
			$maRequette="insert into messages values(null,'".apo($mess)."','".apo($titre)."','$date',0,'$login',null,null);";
			echo $maRequette;
...
<title>Actualités - Tennis de table Ubaye</title>
...
<BODY>
	...
	<center><img src="./images/icones/news.png" id="imgDebutPage"/><span id="titre"> Actualités</span></center></br></br>
	<?php
	 if(isset($_SESSION['root']))
		...
		?>
			...
			<?php if(!$e)
				{
					?><center><h4 id="infoMail"><?php echo $error ?></h4></center><?php
				}

			?>
			<form id="addnews" name="New" action="actualites.php" method="POST" enctype="multipart/form-data" >
			<h4>Nouveau post </h4>
			Titre : <input type="text" name="inputTitre" id="inputTitreadd" /></br>
			Message : </br><TEXTAREA  name="inputMess" rows="10" COLS="100" id="textareaadd"></TEXTAREA>	</br>
			<input type="hidden" value="<?php echo $_SESSION['loginM'] ?>" id="addlogin"/>
		...
		?>
		<?php
			$req="select * from messages where statut=0 ORDER BY id DESC LIMIT 2";
			$rep=mysql_query($req);
			while($ligne=mysql_fetch_array($rep))
			{
				...
				?> 
					...
					<div class="commentaire" id="<?php echo $ligne['id']?>">
					<form name="New2" method="POST" action="actualites.php" enctype="multipart/form-data">
